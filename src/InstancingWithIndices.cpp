...
void InstancingWithIndices::Start()
{
    ...
    CubeVerticeCount = IndexedCubeVertices.size();

    CubeIndices = std::vector<uint32_t>(
        {0, 2, 3,
         0, 3, 1,
         2, 6, 7,
         2, 7, 3,
    ...
    int EboSize = CubeIndices.size() * sizeof(uint32_t);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, EboSize, CubeIndices.data(), GL_STATIC_DRAW);
...
void InstancingWithIndices::Update(float InDeltaTime)
{
    ...
    glDrawElementsInstanced(GL_TRIANGLES,CubeVerticeCount,GL_UNSIGNED_INT,0, CubeCount);
    
}

void InstancingWithIndices::Destroy()
{
