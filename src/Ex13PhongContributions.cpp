...
void Ex13PhongContributions::Start()
{
    ...
    lights_struct.resize(3);

    lights_struct[0].pos = glm::vec3(2,0,0);
    lights_struct[0].color = glm::vec3(1,0,1);
    lights_struct[0].spec_color = glm::vec3(1,1,1);
    lights_struct[0].spec_mul = 1;

    lights_struct[1].pos = glm::vec3(0,4,0);
    lights_struct[1].color = glm::vec3(0,1,0);
    lights_struct[1].spec_color = glm::vec3(1,1,1);
    lights_struct[1].spec_mul = 1;

    lights_struct[2].pos = glm::vec3(-1,0,0);
    lights_struct[2].color = glm::vec3(1,0,0);
    lights_struct[2].spec_color = glm::vec3(1,1,1);
    lights_struct[2].spec_mul = 1;
    for (int i = 0; i < 3; i++)
    {
    ...
    std::cout <<string_passer<<std::endl;
    QuadProgram->SetUniform(string_passer+  ".pos",lights_struct[i].pos);
    std::cout <<string_passer+ ".pos"<<std::endl;

    QuadProgram->SetUniform(string_passer+ ".color",lights_struct[i].color);
    std::cout <<string_passer +  ".color"<<std::endl;

    QuadProgram->SetUniform(string_passer+ ".specular_color",lights_struct[i].spec_color);
    std::cout <<string_passer  +".specular_color"<<std::endl; 

    QuadProgram->SetUniform(string_passer+ ".specular_factor",lights_struct[i].spec_mul);
    std::cout <<string_passer+".specular_factor"<<std::endl;
    }
   
    /*
    QuadProgram->SetUniform("lights[0].pos",lights_struct[0].pos);
    QuadProgram->SetUniform("lights[0].color",lights_struct[0].color);

    QuadProgram->SetUniform("lights[0].specular_color",lights_struct[0].spec_color);

    QuadProgram->SetUniform("lights[0].specular_factor",lights_struct[0].spec_mul);
...
*/
void Ex13PhongContributions::Update(float InDeltaTime)
{
...
void Ex13PhongContributions::Destroy()
{
